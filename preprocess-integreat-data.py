...
def get_data_path(city, language, slug):
    return f'data/integreat-pages/{city}/{language}/{slug}'


# Fetch pages from cms.integreat-app.de
def load_data(city, language):
    os.makedirs(get_data_path(city, language, 'pages'), exist_ok=True)
    response = requests.get(f'https://cms.integreat-app.de/api/{city}/{language}/pages').text
    f = open(get_data_path(city, language, 'pages.json'), 'w')
    f.write(response)


def preprocess(city, language):
    f = open(get_data_path(city, language, 'pages.json'), 'r')
    data = json.load(f)
    chars = 0
    ...
    for page in data:
        # Remove duplicates (duplicated titles)
    ...
    for title, value in page_dict.items():
        ...
        filename = get_data_path(city, language, 'pages/' + str(page_id) + '.txt')
...
if __name__ == '__main__':
    ...
    # load_data('muenchen', 'en')
    preprocess('muenchen', 'en')
